...
2. void enqueue(int element) Adds an element to the front of the queue. If the queue is full, the calling thread is blocked until the queue is no longer full.
3. int dequeue() Returns the element at the rear of the queue and removes it. If the queue is empty, the calling thread is blocked until the queue is no longer empty.
...
that only makes calls to the dequeue method. 
...
public:
    ...
    void enqueue(int element) {
        ...
        cv.wait(lock, [this]{ return q.size() < this->maxCap;});
